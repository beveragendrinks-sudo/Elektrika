...
export default function BCDevisSection({ bcId, bcNumber, category, onReady }: Props) {
  const vendors = getVendorsForCategory(category, 'fournisseur');
  const [quotes, setQuotes] = useState<QuoteRequest[]>(() =>
    MOCK_QUOTE_REQUESTS.filter(q => q.bcId === bcId),
  );
  ...
  function handleSimulateReceive(quoteId: string) {
    const amount = Math.round(800 + Math.random() * 4000);
    setQuotes(prev => {
      const updated = prev.map(q =>
        q.id === quoteId
          ? { ...q, status: 'received' as const, receivedAt: new Date().toISOString(), amount, fileSource: 'email' as const }
          : q,
      );
      const newReceived = updated.filter(q => q.status === 'received' || q.status === 'selected' || q.status === 'rejected');
      onReady?.(newReceived.length >= 2);
      return updated;
    });
  }
  ...
  function handleSelect(quoteId: string) {
    setQuotes(prev => {
      const updated = prev.map(q => ({
        ...q,
        status:
          q.id === quoteId  ? 'selected' as const
          : q.status === 'received' ? 'rejected' as const
          : q.status,
      }));
      onReady?.(true);
      return updated;
  ...
  return (
    <div className="bg-white rounded-xl border border-blue-200 overflow-hidden">
      {/* Header */}
      <div className="px-5 py-3 bg-blue-50 border-b border-blue-200 flex items-start justify-between gap-3">
        <div>
          <div className="font-semibold text-blue-900 text-sm">Appel d&apos;offres fournisseurs — Devis matériaux</div>
          <div className="text-xs text-blue-700 mt-0.5">
            {receivedQuotes.length} devis reçu{receivedQuotes.length !== 1 ? 's' : ''} sur {quotes.length} sollicité{quotes.length !== 1 ? 's' : ''} · Minimum 2 requis pour valider le BC
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          {receivedQuotes.length >= 2 && (
            <button
              ...
              className="text-[10px] px-2.5 py-1 rounded-lg border border-blue-300 text-blue-700 hover:bg-blue-100 transition-colors shrink-0"
            >
        ...
        <>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-slate-400 mt-0.5">
                      ...
                      {q.receivedAt && q.amount && (
                        <>
                          {' · '}
                          <span className="font-semibold text-slate-700">{q.amount.toLocaleString('fr-TN')} TND</span>
                          {q.fileSource && <> · {FILE_SOURCE_ICON[q.fileSource]}</>}
                        </>
                      )}
                      {!q.receivedAt && (
          ...
          {unsolicited.length > 0 && (
            <div className="border-t border-slate-100 px-5 py-3 bg-slate-50">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2.5">
                Fournisseurs agréés non sollicités
              </div>
                ...
                {unsolicited.map(v => (
                  <div key={v.id} className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="text-xs px-3 py-1.5 rounded-lg bg-blue-700 text-white hover:bg-blue-600 disabled:opacity-50 transition-colors shrink-0 flex items-center gap-1.5"
                    >
          ...
          {vendors.length === 0 && (
            <div className="px-5 py-6 text-center text-sm text-slate-400">
              Aucun fournisseur agréé pour cette catégorie — configurez-les dans les Paramètres.
            </div>
          )}

          {quotes.length === 0 && vendors.length > 0 && (
            <div className="px-5 py-6 text-center text-sm text-slate-400">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm border border-slate-200 rounded-lg overflow-hidden">
              <thead className="bg-slate-900 text-white">
                <tr>
                  <th className="text-left px-4 py-2.5 text-xs font-semibold">Fournisseur</th>
      ...
      {selectedQuote && (
        <div className="border-t border-green-200 bg-green-50 px-5 py-3">
          <div className="text-xs text-green-700">
            ✓ Fournisseur retenu : <span className="font-semibold">{selectedQuote.vendorName}</span>
            {selectedQuote.amount && <> · {selectedQuote.amount.toLocaleString('fr-TN')} TND</>}
            {' · '}Le bon de commande peut être validé par le directeur.
          </div>
        </div>
      )}
      ...
      {!selectedQuote && canValidate && (
        <div className="border-t border-emerald-100 bg-emerald-50 px-5 py-3">
          <div className="text-xs text-emerald-700">
            ✓ {receivedQuotes.length} devis reçus — sélectionnez le fournisseur retenu pour débloquer la validation
          </div>
        </div>
      )}
    </div>
